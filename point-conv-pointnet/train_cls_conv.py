...
from utils.utils import save_checkpoint, test
...
def custom_collate_fn(batch):
    ...
    labels = []
    for item in batch:
        point_set, label = item
        points.append(point_set)
        labels.append(label)

    # Ensure all point clouds have the same shape by padding or cropping
    max_points = max(point.shape[0] for point in points)
    points_padded = []
    for point in points:
        if point.shape[0] < max_points:
            # Pad with zeros
            pad_size = max_points - point.shape[0]
            point_padded = np.pad(point, ((0, pad_size), (0, 0)), mode='constant')
        else:
            # Crop
            point_padded = point[:max_points, :]
        points_padded.append(point_padded)

    points_padded = np.stack(points_padded, axis=0)
    labels = np.array(labels)

    # Convert to tensors
    points_tensor = torch.from_numpy(points_padded).float()
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            print("seconddddddddddddddddddddddddddddddddddddddddddddddddd part of data ?/////////////////////////")
            print(points)
            if args.normal:
                pred = classifier(points[:, :3, :], points[:, 3:, :])  # With normals
            else:
                pred = classifier(points[:, :3, :])  # Without normals
